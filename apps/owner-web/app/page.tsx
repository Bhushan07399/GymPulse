    ...
    <main className="min-h-screen bg-[#F8FAFC] text-[#0F172A] flex flex-col justify-between p-4 sm:p-6 lg:p-8">
      ...
      <header className="mx-auto flex w-full max-w-5xl items-center justify-between py-2">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/login"
            className="inline-flex items-center gap-1.5 rounded-xl border border-transparent px-3.5 py-2 text-xs font-bold text-slate-700 transition hover:bg-slate-100 hover:text-slate-900"
          >
            Sign In
          </Link>
          <Link
      ...
      <section className="mx-auto my-auto w-full max-w-5xl py-8">
        <div className="text-center max-w-2xl mx-auto space-y-3 mb-10">
          ...
          <div className="pt-2 flex flex-wrap items-center justify-center gap-3">
            <Link
              href="/login"
              className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-6 py-3 text-xs sm:text-sm font-bold text-white shadow-lg hover:bg-slate-800 transition-all hover:scale-[1.02]"
            >
            ...
            <Link
              href="/create-account"
              className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-6 py-3 text-xs sm:text-sm font-bold text-slate-800 shadow-sm hover:bg-slate-50 transition-all hover:scale-[1.02]"
            >
        ...
        {/* 3D Management Feature Cards Grid */}
        <div className="grid gap-6 md:grid-cols-2">
          {/* CARD 1: OPERATIONS & ATTENDANCE */}
          <motion.div
          ...
          >
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-slate-500 font-medium leading-relaxed">
                Seamless member lifecycle management, plan renewals, manual check-in roster, and digital QR scanner for floor access.
              </p>
            </div>
            ...
            <div className="mt-6 border-t border-slate-100 pt-5">
              ...
              >
                <span>Access Member Desk</span>
                <ChevronRight className="h-4 w-4 text-slate-400 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </motion.div>

          {/* CARD 2: REVENUE, PAYMENTS & MULTI-GYM */}
          <motion.div
            whileHover={{ y: -6, scale: 1.01 }}
            transition={{ duration: 0.25, ease: "easeOut" }}
            className="group relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-7 shadow-xl hover:shadow-2xl transition-all flex flex-col justify-between"
          >
            ...
            <div>
              {/* 3D Visual Floating Badge Container */}
              <div className="relative mb-6 flex h-48 w-full items-center justify-center rounded-2xl bg-gradient-to-br from-blue-700 via-indigo-700 to-slate-900 p-6 text-white overflow-hidden shadow-inner">
                ...
                >
                  ...
                  <span>Branded Receipts & Dues</span>
                </motion.div>
                ...
                <div className="relative text-center z-10">
                  ...
                  <span className="mt-3 block text-xs font-bold tracking-wider text-blue-200 uppercase">
                    Financials & Locations
                  </span>
                </div>
              </div>

              <h2 className="font-extrabold text-2xl text-slate-900 tracking-tight">
                Billing, Receipts & Multi-Gym
              </h2>
              <p className="mt-1 text-xs text-slate-500 font-medium leading-relaxed">
                Record fees, view transaction histories, print professional receipts, and manage multi-gym locations with customizable subscriptions.
              </p>
            </div>
